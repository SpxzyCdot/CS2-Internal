...
#ifndef ImGuiNavHighlightFlags_TypeThin
#define ImGuiNavHighlightFlags_TypeThin 0
#endif
...
#include <string>
    ...
    bool Keybind(const char* label, const char* description, int* key)
    {
        ...
        bool hovered = ItemHoverable(rect, id, ImGuiItemFlags_None);
        ...
        GetWindowDrawList()->AddRectFilledMultiColor(rect.Min, rect.Max - ImVec2(80, 0), GetColorU32(c::elements::background, 0.f), GetColorU32(c::elements::background, 1.f), GetColorU32(c::elements::background, 1.f), GetColorU32(c::elements::background, 0.f));
    ...
    bool edited::Tab(bool selected, const char* icon, const char* label, const char* description, const ImVec2& size_arg)
    {
        ...
        GetWindowDrawList()->AddRectFilledMultiColor(bb.Min + ImVec2(size.x / 2, 0), bb.Max, GetColorU32(state.shadow, 0.f), GetColorU32(state.shadow, 1.f), GetColorU32(state.shadow, 1.f), GetColorU32(state.shadow, 0.f));
    ...
    bool edited::BeginChild(const char* str_id, const ImVec2& size_arg, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags)
    {
        ...
        return edited::BeginChildEx(str_id, id, size_arg, child_flags, window_flags | 0);
    }
    ...
    bool edited::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags)
    {
        ...
        const ImGuiChildFlags ImGuiChildFlags_SupportedMask_ = ImGuiChildFlags_Borders | ImGuiChildFlags_ResizeX | ImGuiChildFlags_ResizeY | ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysAutoResize | ImGuiChildFlags_FrameStyle;
        ...
        if (child_flags & ImGuiChildFlags_FrameStyle)
        {
            ...
            child_flags |= ImGuiChildFlags_Borders | 0;
            window_flags |= ImGuiWindowFlags_NoMove;
        }

        // g.NextWindowData.Flags &= ~ImGuiNextWindowDataFlags_HasSize; // Flags member missing
   ...
   //     GetWindowDrawList()->AddRect(parent_window->DC.CursorPos, parent_window->DC.CursorPos + size_arg, GetColorU32(c::accent), 0);
    ...
    bool edited::Checkbox(const char* label, const char* description, bool* v)
    {
        ...
        GetWindowDrawList()->AddRectFilledMultiColor(rect.Min, rect.Max - ImVec2(20, 0), GetColorU32(c::elements::background, 0.f), GetColorU32(c::elements::background, 1.f), GetColorU32(c::elements::background, 1.f), GetColorU32(c::elements::background, 0.f));
    ...
    bool SliderScalar(const char* label, const char* description, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
    {
        ...
        bool hovered = ItemHoverable(frame_bb, id, g.LastItemData.ItemFlags), held, pressed = ButtonBehavior(frame_bb, id, &hovered, &held, NULL);
        ...
        GetWindowDrawList()->AddRectFilledMultiColor(rect.Min, rect.Max - ImVec2(120, 0), GetColorU32(c::elements::background, 0.f), GetColorU32(c::elements::background, 1.f), GetColorU32(c::elements::background, 1.f), GetColorU32(c::elements::background, 0.f));
    ...
    bool BeginCombo(const char* label, const char* description, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
    {
        ...
        // AddShadowCircle not found, using AddCircle
        // The original line was syntactically incorrect and likely a placeholder.
        // Assuming 'center' and 'radius' are defined elsewhere or this line was meant to be removed.
        // For now, I'm commenting it out as it's not a valid ImGui draw call without context.
         // Shadow color fixed
         // window->DrawList->AddCircle(center, radius, IM_COL32(0, 0, 0, 100), 12, 2.0f);
        ...
        GetWindowDrawList()->AddRectFilledMultiColor(bb.Min, bb.Max - ImVec2(120, 0), GetColorU32(c::elements::background, 0.f), GetColorU32(c::elements::background, 1.f), GetColorU32(c::elements::background, 1.f), GetColorU32(c::elements::background, 0.f));

    // AddShadowRect not found, using AddRect
        window->DrawList->AddRect(rect.Min + ImVec2(2, 2), rect.Max + ImVec2(2, 2), IM_COL32(0, 0, 0, 100), c::elements::rounding);
        ...
        GetWindowDrawList()->AddRectFilledMultiColor(rect.Min, rect.Max - ImVec2(30, 0), GetColorU32(c::elements::background_widget, 0.f), GetColorU32(c::elements::background_widget, 1.f), GetColorU32(c::elements::background_widget, 1.f), GetColorU32(c::elements::background_widget, 0.f));
    ...
    bool Combo(const char* label, const char* description, int* current_item, const char* (*getter)(void* user_data, int idx), void* user_data, int items_count, int popup_max_height_in_items)
    {
        ...
        for (int i = 0; i < items_count; i++)
        {
            ...
            ImGui::PushID(i);
            if (edited::Selectable(item_text, is_selected))
            {
                *current_item = i;
                value_changed = true;
            }
    ...
    bool Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
    {
        ...
        if (pressed || (hovered && (flags & ImGuiSelectableFlags_SetNavIdOnHover)))
        {
            if (/* !g.NavDisableMouseHover && */ g.NavWindow == window && g.NavLayer == window->DC.NavLayerCurrent)
            {
                ...
                // g.NavDisableHighlight = true; // Member missing
            }
        }
        ...
        if (g.NavId == id) {
        // ImGuiNavHighlightFlags_TypeThin might be missing in this ImGui version
        #ifndef ImGuiNavHighlightFlags_TypeThin
        #define ImGuiNavHighlightFlags_TypeThin 0
        #endif
        ...
        // ImGuiItemFlags_SelectableDontClosePopup might be missing
        #ifndef ImGuiItemFlags_SelectableDontClosePopup
        #define ImGuiItemFlags_SelectableDontClosePopup 0
        #endif
        ...
        if (pressed && (window->Flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiSelectableFlags_DontClosePopups) && !(g.LastItemData.ItemFlags & ImGuiItemFlags_SelectableDontClosePopup)) CloseCurrentPopup();
    ...
    bool QuandScalar(const char* label, const char* description, ImGuiDataType data_type, void* p_data0, void* p_data1, const void* p_min0, const void* p_max0, const void* p_min1, const void* p_max1, const char* format0, const char* format1, ImGuiSliderFlags flags)
    {
        ...
        window->DrawList->AddRect(ImVec2(it_anim->second.slow0 + frame_bb.Min.x, grab_bb0.Min.y), ImVec2(it_anim->second.slow0 + frame_bb.Min.x, grab_bb0.Max.y), IM_COL32(0, 0, 0, 100), 25.f, ImDrawFlags_None, 100.f);
        ...
        window->DrawList->AddRect(ImVec2(grab_bb1.Min.x, it_anim->second.slow1 + frame_bb.Min.y), ImVec2(grab_bb1.Max.x, it_anim->second.slow1 + frame_bb.Min.y), IM_COL32(0, 0, 0, 100), 25.f, ImDrawFlags_None, 100.f);
    ...
    bool color_button(const char* name, ImVec2 size, ImU32 color_bg)
    {
        ...
        window->DrawList->AddCircle(rect.Min + ImVec2(size / 2), size.x / 2, IM_COL32(0, 0, 0, 100), 18.f, 30.f);
    ...
    bool ColorEdit4(const char* label, const char* description, float col[4], ImGuiColorEditFlags flags)
    {
        ...
        if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
        {
            ...
            if (ItemHoverable(g.LastItemData.Rect, g.LastItemData.ID, ImGuiItemFlags_None) && g.IO.MouseClicked[0] || state.active && !search_col && g.IO.MouseClicked[0] && !state.hovered)
